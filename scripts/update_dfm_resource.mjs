...
import fetch from 'node-fetch'; // Polyfill or native
...
async function updateResource() {
  const url = \`\${supabaseUrl}/rest/v1/resources?slug=eq.checklist-dfm-implantes\`;
  ...
  if (!response.ok) {
    ...
    console.error("Error updating resource:", response.status, text);
  } else {
    const data = await response.json();
    console.log("Successfully updated checklist DFM!");
    console.log(data.length > 0 ? "Rows updated: " + data.length : "No rows found to update.");
  }
}

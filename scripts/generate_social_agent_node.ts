/**
 * Multi-Agent Creative Pipeline for Lifetrek Medical (Node.js Version)
 * 
 ...
 * 
 * Usage:
 *   npx tsx scripts/generate_social_agent_node.ts "Precisão CNC em Implantes"
 *   npx tsx scripts/generate_social_agent_node.ts "Precisão CNC" --dry-run
 */
...
// Fix: Use SUPABASE_URL from env, handled correctly
...
Avalie a qualidade total.`;
    ...
    try {
        ...
        // Handle initial response for background processing
        const text = await imageResponse.text();
        let imageResult;
        try {
            imageResult = JSON.parse(text);
        } catch (e) {
            // If it's not JSON, it might be an error page or simple status
            console.log(`   ℹ️  Response text: ${text.slice(0, 100)}...`);
            imageResult = { success: imageResponse.ok };
        }

        if (imageResponse.ok) {
            console.log(`   ✅ Edge Function triggered successfully.`);
            if (imageResult.images_generated) {
            ...
            console.error(`   ⚠️  Geração falhou: ${imageResult.error || imageResponse.statusText}`);
        }
    } catch (e) {
...
const topic = args.filter((a: string) => !a.startsWith("--"))[0] || "Manufatura Sustentável de Dispositivos Médicos";

    ...
    <body class="bg-gray-900 text-white flex items-center justify-center min-h-screen">
        ...
        <div id="slide" class="relative w-[1080px] h-[1350px] overflow-hidden bg-gradient-to-br from-[#0a192f] to-[#004daa] flex flex-col p-20">
            ...
            <div class="relative z-10 flex justify-between items-start mb-20">
                ...
                <!-- ISO BADGE INJECTION -->
                ${isoBadgeDataUrl ? `<img src="${isoBadgeDataUrl}" class="iso-badge" alt="ISO 13485" />` : ''}
            </div>
...
// ─── Main Execution ─────────────────────────────────────────────────────────
async function run() {
    console.log("🚀 Starting Local Playwright Generation...");
    ...
    // Set viewport to match slide dimensions (optional, but good practice)
    await page.setViewportSize({ width: 1080, height: 1350 });

    // 4. Generate Slides
    for (let i = 0; i < SLIDES_CONTENT.length; i++) {
        const slide = SLIDES_CONTENT[i];
        console.log(`🎨 Generating Slide ${i + 1}: ${slide.title}...`);

        const htmlContent = generateHtml(slide, isoBadge);

        // Write temp HTML for debugging (optional)
        // fs.writeFileSync(path.join(outputDir, `slide_${i+1}.html`), htmlContent);

        await page.setContent(htmlContent);

        // Wait specifically for the image to satisfy 'visible' if it exists.
        // Or just wait for network idle to ensure external fonts/bg load.
        await page.waitForLoadState('networkidle');

        const outputPath = path.join(outputDir, `slide_${i + 1}.png`);

        // Screenshot specifically the #slide element to avoid whitespace if viewport differs
        const element = await page.$('#slide');
        if (element) {
            await element.screenshot({ path: outputPath });
            console.log(`   ✅ Saved to: ${outputPath}`);
        } else {
            console.error(`   ❌ Could not search #slide element.`);
        }
    }

    await browser.close();
    console.log("\n✅ Generation Complete! Check 'generated_content/' folder.");
}

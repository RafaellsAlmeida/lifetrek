...
interface KnowledgeBaseCoreProps {
    embedded?: boolean;
}

export function KnowledgeBaseCore({ embedded = false }: KnowledgeBaseCoreProps) {
    ...
    return (
        <div className={`${embedded ? '' : 'container max-w-5xl py-8'} space-y-6 animate-fade-in`}>
            {!embedded && (
            ...
            {isLoading ? (
                ...
                <div className="space-y-6">
                    {Object.entries(groupedItems).map(([category, items]) => (
                        <Card key={category} className={embedded ? "border-none shadow-none bg-transparent" : ""}>
                            <CardHeader className={`py-4 ${embedded ? 'px-0' : 'bg-muted/30'}`}>
                                <CardTitle className="text-lg font-medium text-primary">{category}</CardTitle>
                            </CardHeader>
                            <CardContent className={embedded ? "p-0" : "p-0"}>
                                <Accordion type="single" collapsible className="w-full">
                                    {items.map((item) => (
                                        <AccordionItem key={item.id} value={item.id} className={embedded ? "px-0" : "px-4"}>
